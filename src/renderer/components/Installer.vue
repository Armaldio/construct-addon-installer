  ...
  export default {
    ...
    computed: {
      addonToList() {
        ...
        const list = [];
        for (let i = 0; i < Object.keys(this.addon).length; i += 1) {
          const key = Object.keys(this.addon)[i];
    ...
    methods: {
      ...
      async install() {
        const version = await this.checkInstalled(this.c2addonsPath);
        ...
        return new Promise((resolve/* , reject */) => {
          new JSZip.external.Promise((zipresolve, zipreject) => {
            ...
            Object.keys(this.jsZipInstance.files).forEach(async (filename) => {
              const f = path.relative('files', filename);
              if (f !== '' && f !== '..\\info.xml' && path.extname(f) === '') {
                console.log(`-> ${f}`);
                console.log('type', this.addon.type);

                const expectedDestination = path.join(this.c2addonsPath, `${this.addon.type}s`, f);
                console.log(`Expected dest: ${expectedDestination}`);
                // todo get version
                const ace = ap.export(expectedDestination, 'json');
                console.log(ace);
      ...
      async extract() {
        ...
        for (let i = 0; i < Object.keys(this.jsZipInstance.files).length; i += 1) {
          const filename = Object.keys(this.jsZipInstance.files)[i];

          if (filename !== 'files/' && filename !== 'info.xml') {
            // eslint-disable-next-line
            const content = await this.jsZipInstance.files[ filename ].async('nodebuffer');
            const f = path.relative('files', filename);

            const dest = path.join(this.c2addonsPath, `${this.addon.type}s`, f);
    ...
    async mounted() {
      // Check for updates
      const autoUpdater = this.$electron.remote.getGlobal('autoUpdater');
      ...
      try {
        ...
        const p = await this.download(this.link);
        ...
        /* this.addon = {
            ...
            'documentation': '',
            'description'  : 'A Construct 2 behavior to enhance the platform behavior
            and make the character follow the ground angle accurately.'
        }; */
